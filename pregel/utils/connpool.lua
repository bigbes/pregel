...
local function connect(self, id, server)
    ...
    while true do
        ...
        if login == nil or (login == 'guest' and pass == nil) then
            login = ''
        else
            login = login .. ':' .. pass .. '@'
        end
        local uri = login .. server.uri
...
local function init(self, cfg)
    ...
    if cfg.monitor == nil or cfg.monitor then
        fiber.create(heartbeat_fiber, self)
        fiber.create(monitor_fiber, self)
    end
